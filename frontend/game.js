...
// Wrapped in function to prevent global scope pollution and handle socket dependency
function setupBossMechanics() {
    if (!window.socket) {
        // Socket might not be initialized yet, retry shortly
        setTimeout(setupBossMechanics, 500);
        return;
    }
...
// Load GLB model from URL or R2 key
...
async function loadWeaponGLB(weaponKey, type) {
    ...
    // FIX: Gracefully handle missing optional assets (e.g. 8x bullet)
    if (!filename) {
        return null;
    }
